...
def inicializiraj_bazo():
    ...
    if not cur.fetchone():
        cur.execute("INSERT INTO baza_uporabnikov (ime, email, geslo, admin) VALUES (?, ?, ?, ?)",
                    ("tino", "admin@email.com", "admin123", 1))
        con.commit()
...
def registracija():
    if request.method == "POST":
        ...
        cur.execute("INSERT INTO baza_uporabnikov (ime, email, geslo) VALUES (?, ?, ?)", 
